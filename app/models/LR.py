...
def main(data_size, test_size=0.2, min_df=5, vocab_model='unigram', tf_idf=False, corrected=False):
	...
	if tf_idf:
		...
		if vocab_model == 'bigram':
			vect = TfidfVectorizer(min_df=min_df, stop_words="english", ngram_range=(2, 2)).fit(text_train)
		if vocab_model == 'trigram':
			vect = TfidfVectorizer(min_df=min_df, stop_words="english", ngram_range=(3, 3)).fit(text_train)
	else:
		if vocab_model == 'unigram':
			vect = CountVectorizer(min_df=min_df, stop_words="english", ngram_range=(1, 1)).fit(text_train)
		if vocab_model == 'bigram':
			vect = CountVectorizer(min_df=min_df, stop_words="english", ngram_range=(2, 2)).fit(text_train)
		if vocab_model == 'trigram':
			vect = CountVectorizer(min_df=min_df, stop_words="english", ngram_range=(3, 3)).fit(text_train)
	...
	if tf_idf:
		joblib.dump(clf, '%s/../data/models/LR_%s_tfidf.pkl' % (dir_path, vocab_model))
		joblib.dump(vect.vocabulary_, '%s/../data/vocabulary/%s_tfidf.pkl' % (dir_path, vocab_model))
	else:
		joblib.dump(clf, '%s/../data/models/LR_%s.pkl' % (dir_path, vocab_model))
		joblib.dump(vect.vocabulary_, '%s/../data/vocabulary/%s.pkl' % (dir_path, vocab_model))

	# print(explain_prediction.explain_prediction_linear_classifier(clf, text_train[0], vec=vect, top=10, target_names=reviews.target_names).targets)
	if data_size == -1:
