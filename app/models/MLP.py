...
def main(data_size, test_size=0.2, min_df=5, vocab_model='unigram', tf_idf=False, corrected=False):
	...
	clf = MLPClassifier(hidden_layer_sizes=(100,), activation='logistic', random_state=1, verbose=True)
	...
	if tf_idf:
		joblib.dump(clf, '%s/../data/models/MLP_%s_tfidf.pkl' % (dir_path, vocab_model))
		joblib.dump(vect.vocabulary_, '%s/../data/vocabulary/%s_tfidf.pkl' % (dir_path, vocab_model))
	else:
		joblib.dump(clf, '%s/../data/models/MLP_%s.pkl' % (dir_path, vocab_model))
